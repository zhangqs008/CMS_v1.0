...
namespace HC.Repository
{
    ...
    public class Database : IDisposable
    {
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, sql, args))
                    {
                        ...
                        {
                            return default(T);//guocheng修改，防止字段值为null时导致异常，直接返回默认值 
                        }
                    }
                }
                finally
        ...
        public static bool SplitSqlForPaging(string sql, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
        {
            ...
            {
                // sqlCount = sql.Substring(0, g.Index) + "COUNT(" + m.Groups[1].ToString().Trim() + ") " + sql.Substring(g.Index + g.Length);//原始
                //2013-04-28 修改
                sqlCount = "SELECT COUNT(*) COUNTRECORD FROM (" + sql + ") WHIRCONT";
            }
            else
            {
                sqlCount = sql.Substring(0, g.Index) + "COUNT(*) COUNTRECORD " + sql.Substring(g.Index + g.Length);
                // sqlCount = "SELECT COUNT(*) COUNTRECORD FROM (" + sql + ") WHIRCONT";
            }
        ...
        public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            ...
            if (_dbType == DBType.SqlServer || _dbType == DBType.Oracle)
            {
                ...
                sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                                        sqlOrderBy == null ? "ORDER BY (SELECT NULL)" : sqlOrderBy, sqlSelectRemoved, ArgsLength, ArgsLength + 1);
                args = args.Concat(new object[] { skip, skip + take }).ToArray();
            }
            else if (_dbType == DBType.SqlServerCE)
            {
                sqlPage = string.Format("{0}\nOFFSET @{1} ROWS FETCH NEXT @{2} ROWS ONLY", sql, ArgsLength, ArgsLength + 1);
                args = args.Concat(new object[] { skip, take }).ToArray();
            }
            else
            {
                sqlPage = string.Format("{0}\nLIMIT @{1} OFFSET @{2}", sql, ArgsLength, ArgsLength + 1);
...
//#endif
        ...
        #region 扩展出DataSet  add by gc 2012-7-15
        /// <summary>
        /// 执行查询，返回DataSet
        /// </summary>
        /// <param name="sql">SQL语句</param>
        ...
        /// <returns></returns>
        public DataSet Query(string sql, params object[] args)
        {
            OpenSharedConnection();
            ...
            try
            {
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    //if (_dbType == DBType.SqlServer || _dbType == DBType.MySql)
                    //{
                    ...
                    //{
                    //    #region 方法一
                    //    //using (Oracle.DataAccess.Client.OracleDataAdapter oda = new Oracle.DataAccess.Client.OracleDataAdapter())
                    //    //{
                    //    //    oda.SelectCommand = (Oracle.DataAccess.Client.OracleCommand)cmd;
                    //    //    oda.Fill(ds);
                    //    //}
                    //    #endregion

                    //    #region 方法二
                    //    // dataset的DbDataAdapter填充不知道为何在oracle下有异常，dbDataAdapter为null,暂使用DataReader填充
                    //    IDataReader r;
                    //    try
                    //    {
                    //        r = cmd.ExecuteReader();
                    ...
                    //    using (r)
                    //    {
                    //        ds.Load(r, LoadOption.OverwriteChanges, new string[] { "Result" + DateTime.Now.Millisecond.ToString() });
                    //    }
                    //    #endregion
                    //}
                }
        ...
        /// <returns></returns>
        public PageDataSet CustomQuery(long page, long itemPerPage, string sql, params object[] args)
        {
            OpenSharedConnection();


            PageDataSet Result = new PageDataSet();
            Result.CurrentPage = page;
            Result.ItemsPerPage = itemPerPage;
            Result.TotalPages = Result.TotalItems / itemPerPage;
            if ((Result.TotalItems % itemPerPage) != 0)
                Result.TotalPages++;
            ...
            if (_dbType == DBType.SqlServer || _dbType == DBType.Oracle)
            {
                Result.TotalItems = DbHelper.CurrentDb.ExecuteScalar<int>(string.Format(" SELECT Count(*) FROM ({0}) peta_paged ", sql), args);

                sql = string.Format(" SELECT * FROM ({0}) peta_paged WHERE peta_rn>{1} AND peta_rn<={2}  ", sql, itemPerPage * (page - 1), itemPerPage * page);


            }
            else if (_dbType == DBType.MySql)
            {
                Result.TotalItems = DbHelper.CurrentDb.ExecuteScalar<int>(string.Format(" SELECT Count(*) FROM ({0})  ", sql), args);
                sql = string.Format("  ({0})    \nLIMIT  {1} OFFSET  {2}", sql, itemPerPage, itemPerPage * (page - 1));
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public PageDataSet Query(long page, long itemPerPage, string sql, params object[] args)
        {
            OpenSharedConnection();

            //  sql = "SELECT TT.* FROM ("+sql+") TT";//防止sql存在top时报错,后台整合添加
            ...
            if (_dbType == DBType.SqlServer)
            {
                ...
                sql = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>{2} AND peta_rn<={3}",
                                        sqlOrderBy == null ? "ORDER BY (SELECT NULL)" : sqlOrderBy, sqlSelectRemoved, itemPerPage * (page - 1), itemPerPage * page);
            }
            else if (_dbType == DBType.Oracle)
            {
                ...
                sql = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>{2} AND peta_rn<={3}",
                                        sqlOrderBy == null ? "ORDER BY ( NULL)" : sqlOrderBy, sqlSelectRemoved, itemPerPage * (page - 1), itemPerPage * page);
            }
            else
            {
                sql = string.Format("  ({0})    \nLIMIT  {1} OFFSET  {2}", sql, itemPerPage, itemPerPage * (page - 1));
            ...
            try
            {
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    //    using (r)
                    //    {
                    //        ds.Load(r, LoadOption.OverwriteChanges, new string[] { "Result" });
                    //    }
                    //}
