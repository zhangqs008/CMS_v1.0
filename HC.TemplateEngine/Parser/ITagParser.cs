...
namespace HC.TemplateEngine.Parser
{
    ...
    public class ForParser : ITagParser
    {
        ...
        public Tag Parse(TemplateParser parser, TokenCollection tc)
        {
            ...
            if (tc.Count > 3 && Common.ParserHelpers.IsEqual(Field.KEY_FOREACH, tc.First.Text))
            {
    ...
    public class ComplexParser : ITagParser
    {
        ...
        public Tag Parse(HC.TemplateEngine.Parser.TemplateParser parser, TokenCollection tc)
        {
            if (tc.Count > 2)
            {
                Int32 start, end, pos;
